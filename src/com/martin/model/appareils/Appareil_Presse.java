...
public class Appareil_Presse extends Appareil {
	...
	public Appareil_Presse(int x, int y, String rotate) {
		super(x, y, rotate);
			...
			if(X>-1 && Y>-1 && X<Stats.largeurGrille && Y<Stats.longueurGrille) {
				for(int j = 0; j < JeuContrôle.images[X][Y].getAppareil().getEntréesList().size(); j++) {
					...
					if(sorties.get(i) == JeuContrôle.images[X][Y].getAppareil().getEntréesList().get(j)&& ressources.size() > 0) {
						...
						case "Fer":
							res = Ressource.PLAQUE_DE_FER;
							break;
						case "Or":
							res = Ressource.PLAQUE_DE_OR;
							break;
						case "Cuivre":
							res = Ressource.PLAQUE_DE_CUIVRE;
							break;
						case "Argent":
							res = Ressource.PLAQUE_DE_ARGENT;
							break;
						case "Aluminium":
							res = Ressource.PLAQUE_DE_ALUMINIUM;
							break;
