...
public final class GameController {
     ...
     */
    public GameController(GameView gameController, Game game) {
	// Sets the dimension
    ...
    /**
     * See {@link GameView#lightViewRefresh(Coordinate)}.
     * 
    ...
    /**
     * See {@link GameView#hardViewRefresh(Coordinate)}.
     * 
     * @param coord the location
     */
     ...
     * {@link com.martinheywang.products.controller.DeviceController#connectionExists(Coordinate, Coordinate)}.
     * </p>
     * 
	...
	public void run() {
		...
		try {
		    for (final Device independent : GameController.this.deviceController.getIndependentDevices())
			for(int i = 0; i < independent.getLevel().getValue(); i++) {
				...
				while (action.isSuccessful()) {
				    ...
				    // If the device doesn't give any resources and an output, the assembly line
				    // reached the end.
				    if (action.getGivenResources() == null)
					if (action.getOutput() == null) {
					    // So we break the loop and set the operation to successful
					    success = true;
					    break;
					}
				    ...
				    if (action.getOutput() == null
					    || !GameController.this.connectionExists(action.getPosition(), action.getOutput()))
					break;
				    ...
				    action = next.act(action.getGivenResources());
