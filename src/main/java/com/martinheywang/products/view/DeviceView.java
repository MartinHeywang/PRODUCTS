...
public final class DeviceView extends ImageView {
	...
	static final DataFormat classFormat = new DataFormat("class");
	...
	public DeviceView(Device device) {
		...
		this.device.activeProperty().addListener((observableValue, oldValue, newValue) -> {
			if (newValue == true) {
				// Show the new active state with a smooth transition
				final Timeline in = new Timeline();
				in.getKeyFrames().addAll(
						new KeyFrame(Duration.ZERO, new KeyValue(this.opacityProperty(), defaultOpacityValue)),
						new KeyFrame(Duration.millis(1200), new KeyValue(this.opacityProperty(), defaultOpacityValue)));
				in.playFromStart();
			} else {
				// Unshow the active state
				final Timeline out = new Timeline();
				out.getKeyFrames().addAll(
						new KeyFrame(Duration.ZERO, new KeyValue(this.opacityProperty(), activeOpacityValue)),
						new KeyFrame(Duration.millis(1200), new KeyValue(this.opacityProperty(), defaultOpacityValue)));
				out.playFromStart();
			}
		});
			...
			try {
				if (event.getTarget().getClass().equals(DeviceView.class)) {
					if (event.getTransferMode().equals(TransferMode.LINK)) {
						// Quick review : LINK to swap devices
						device.swap();
					}
				}
