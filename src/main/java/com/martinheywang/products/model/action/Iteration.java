...
public final class Iteration {
     ...
     * iteration.
     */
    private final LocatedHashMap<Coordinate, Pack> received;

    /**
     * A Map representing what the device gave, to where, during the iteration.
     */
    private final LocatedHashMap<Coordinate, Pack> given;
	...
	return this.device.getLevel().getValue() * this.device.getEntriesCount() * 2;
    }
	...
	for (final Coordinate key : this.received.keySet())
	    if (key.propertiesEquals(from))
		for (final Pack item : this.received.get(key))
		    if (item.getResource().equals(pack.getResource())) {
			item.addQuantity(pack.getQuantity());
			return;
		    }
	this.received.put(from, new Pack(pack.getResource(), pack.getQuantity()));
    }
	...
	// Tries to merge the given to another that may exist
	for (final Coordinate key : this.given.keySet())
	    if (key.propertiesEquals(to))
		for (final Pack item : this.given.get(key))
		    if (item.getResource().equals(pack.getResource())) {
			item.addQuantity(pack.getQuantity());
			return;
		    }
	...
	// reference)
	this.given.put(to, new Pack(pack.getResource(), pack.getQuantity()));
    }
